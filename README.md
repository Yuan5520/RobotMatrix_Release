...
1. 在场景中准备机械臂模型，建议按“基座到末端”的层级整理关节 Transform。
2. 在机械臂根节点或控制节点上添加 `RobotArmBehaviour`。
3. 在 `Joint Transforms` 中按运动学链顺序填入每个关节 Transform。
4. 在 `End Effector Transform` 中填入 TCP 或末端工具 Transform。
5. 在 `Joint Configs` 中配置每个关节的 DH 参数、旋转轴、关节限位和速度能力。
6. 在 `IK Config` 中使用默认参数启动，再按机械臂表现逐步调参。
7. 在 `运行时参数` 中确认默认速度：FK `30 deg/s`、IK 移动 `0.2 m/s`、IK 旋转 `45 deg/s`、插值最大角速度 `60 deg/s`。
8. 进入 Play Mode，通过键盘、鼠标、Scene 手柄或 C# API 控制机械臂。
